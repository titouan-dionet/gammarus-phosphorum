...
run_single_simulation <- function(
  ...
  for (el in element_names) {
    results[[paste0("mean_percent", el)]] <- percent_pop[[paste0(
      "percent",
      el
    )]]
...
#' @export
generate_single_param_combinations <- function(
  temps,
  classes,
  base_surv_rates,
  n_iter = 1000
...
#' @export
generate_multi_param_combinations <- function(
  temps,
  n_iter = 10000,
  min_val = 0.001,
  max_val = 1
...
#' @export
sample_elasticity_parameters <- function(
  multi_param_results,
  n_samples = 10000
) {
  # Set random seed for reproducibility
...
calculate_elasticity <- function(
  ...
  for (j in seq_along(class_names)) {
    ...
    if (parameter_type == "survival") {
      ...
      for (i in 1:nrow(transition_matrix)) {
        if (i == j) {
          ...
          mod_transition_matrix[j, j] <- transition_matrix[j, j] +
            reduction_amount
        } else if (i > j) {
    ...
    mod_percentP <- e_rates[['percent_elem_pop_biomass']][[paste0(
      "percent",
      element_names[1]
    )]]
...
#' @return A data.table with elasticity results
#' @export
calculate_model_parameter_elasticity <- function(
  param_set,
  class_names,
  transition_matrix,
  fecondity_matrix,
